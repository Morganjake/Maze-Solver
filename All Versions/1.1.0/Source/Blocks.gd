...
func Calculate(IsMaze):
	...
	while TotalChecked <= XSize * YSize - WallCount: # Loops while all the blocks haven't been checked
		...
		for Block in NextBlocks: # Loops all the neighbours of the previous neighbours
			...
			if AllowDiagonal == true:
				for DiagonalNeighbour in Block.DiagonalNeighbours:
					if DiagonalNeighbour.Checked == false and DiagonalNeighbour.Wall == false: # Only adds the neighbour to a list if it's not a wall or already checked
						var BitwiseAnd = 0 # Stores all neighbours of both the block and it's diagonal neighbour 
						for Neighbour in Block.Neighbours:
							if Neighbour in DiagonalNeighbour.Neighbours and Neighbour.Wall == false:
								BitwiseAnd += 1
						
						if BitwiseAnd >= 1: # Either on of the neighbours aren't walls
							Paths[DiagonalNeighbour] = Paths[Block].duplicate()       
							Paths[DiagonalNeighbour].append(DiagonalNeighbour)
							DiagonalNeighbour.Checked = true
							Neighbours.append(DiagonalNeighbour)
	...
	if not EndBlock.Checked and IsMaze == true:
		_on_create_maze_pressed()
...
func _on_allow_diagonal_pressed():
	AllowDiagonal = true if AllowDiagonal == false else false
	if ToggledCalculate == true:
		Calculate(false)
